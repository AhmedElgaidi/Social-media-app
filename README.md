...
9. Forget Password.
10. Reset Password.
11. Update session tokens (refresh token = new access token + new refresh token).
12. Get all active sessions (Every session = access token + refresh token + device information).
13. Enable/ disable TOTP as 2FA.
14. Enable/ disable OTP as 2FA.
15. Resend OTP.
16. Enable/ disable SMS as 2FA.
17. Resend SMS.
18. Enable/ disable security question.
19. Update/ change security question.
20. Enable/ disable backup codes as account recovery method.
21. Enable/ disable trusted (secondary) email as account recovery method.
...
- **For:** The user can know the status of this feature (Is this featus enabled or not? What is the trusted secondary email associated to his acocunt?)
- **Requirments:** Just a valid access token (or at least a valid refresh token that brings him valid access token).
- **Result:** More control over his account.

#### (42) Enable:

- **For:** The user can enable this feature by addin a valid email.
- **Requirments:** Just a valid access token (or at least a valid refresh token that brings him valid access token) + valid email.
- **Result:** email sent to the mailbox of this secondary email.

#### (43) Change:

- **For:** The user can change the trusted (secondary) email whenever he wants.
- **Requirments:** Just a valid access token (or at least a valid refresh token that brings him valid access token) + prsence of old verified account.
- **Result:** More control over his account.

#### (44) Verify:

- **For:** After the user clicks on the link sent to his secondary mailbox, we verify it and if everything is okay, we enable this feature in his account.
- **Requriments:** Access to his secondary mailbox.
- **Result:** Enabling trusted email recovery method and becoming less risky to lose account.

#### (45) Disable:

- **For:** The user can disable this feature whenever he wants.
- **Requirments:** Just a valid access token (or at least a valid refresh token that brings him valid access token).
- **Result:** the feature is disabled and his account becomes more risky to be lost in the future.

#### (46) send email to trusted(secondary) during recovery:

- **For:** This is considered the method the user can use to recovery his/ her account after all wrong login/ recovery attempts.
- **Requirments:** Just this feature should be enbaled.
- **Result:** The user can recoveyr his account even if he lost his password, can't use all the enabled 2FA methods and even the first recovery method (backup codes).

#### (47) Verify email sent:
